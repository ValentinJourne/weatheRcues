...
climwin_site_days <- function(climate_data,
  ...
  test_inputs <- function() {
    ...
    if (!(inherits(climate_data, "data.frame") || inherits(climate_data, "tbl_df"))) {
    ...
    if (length(missing_columns) > 0) {
      stop(paste("climate_data is missing the required columns of interest:", paste(missing_columns, collapse = ", ")))
    }
    ...
    if (!is.numeric(refday) || length(refday) != 2) stop("refday must be a numeric vector of length 2")
    if (!optionwindows %in% c('absolute', 'relative')) {
      stop("optionwindows must be either 'absolute' or 'relative'. More details can be find in climwin vignette (https://cran.r-project.org/web/packages/climwin/vignettes/climwin.html, sep 2024)")
    }
    if (!stat.aggregate %in% c('mean', 'sum', 'min', 'max')) {
      stop("stat.aggregate must be one of 'mean', 'sum', 'min', or 'max'. More details can be find in climwin vignette (https://cran.r-project.org/web/packages/climwin/vignettes/climwin.html, sep 2024)")
    }
    if (!inherits(try(as.formula(formulanull), silent = TRUE), "formula")) {
      stop("formulanull must be a valid formula in the form of 'response ~ predictor'. More details can be find in climwin vignette (https://cran.r-project.org/web/packages/climwin/vignettes/climwin.html, sep 2024)")
    }
